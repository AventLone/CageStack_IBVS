...
它依赖原始源点 $p_i$ 与当前旋转 $R$，而不是变换后的点 $Tp_i$。这是 ESKF 内部迭代更新使用的扰动形式；独立 `GICP` 实现仍使用上一节的左扰动。右扰动实现时必须成对使用：
...
对于纯扫描到地图 GICP，两种约定都可用。本仓库中的独立 `GICP` 保持左扰动；ESKF 内部的迭代激光更新则使用右旋转误差。两者分别保持 Jacobian 和位姿更新的配对，不共享线性化接口。
