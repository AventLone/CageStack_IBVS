    ...
    : mConfig(config), mTi2l(T_IL)
{
    ...
    for (const double value : {config.max_neighbor_distance, config.max_plane_distance,
                        config.max_planarity_ratio, config.max_residual, config.lidar_noise, config.huber_scale})
    {
        if (!std::isfinite(value) || value <= 0.0)
        {
            throw std::invalid_argument("LiDAR thresholds must be positive and finite.");
        }
    }
}

ESKF::Measurement LidarMeasurement::build(const ImuState& state, const pcl::PointCloud<pcl::PointXYZ>& scan, const SparseVoxel& map) const
{
    ...
    for (const auto& point : scan)
    {
        ...
        if (neighbors.size() < static_cast<std::size_t>(mConfig.neighbors))
        {
            continue;
        }
        ...
        {
            if (neighbor.squared_distance > mConfig.max_neighbor_distance * mConfig.max_neighbor_distance)
            {
                valid = false;
            }
        ...
        if (solver.info() != Eigen::Success || solver.eigenvalues()[1] < 1e-5 ||
            solver.eigenvalues()[0] > mConfig.max_planarity_ratio * solver.eigenvalues()[1])
        {
            continue;
        }
        ...
        if (std::abs(constraint.residual) > mConfig.max_residual)
        {
            continue;
        }
