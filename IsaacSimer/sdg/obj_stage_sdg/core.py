...
class ObjectStageGenerator:
    ...
    def create_stage(self, width=42, length=42):
        self.width = width
        self.length = length
        ...
        prims.create_prim(f"/World/env_{self.env_id}", prim_type="Xform")
        ...
        for x in np.arange(x_min, x_max, 6):
            for y in np.arange(y_min, y_max, 6):
                ...
                floor_prim = prims.create_prim(
                    prim_path=f"/World/env_{self.env_id}/Floor_{count}",
                    position=position,
                ...
                ceil_prim = prims.create_prim(
                    prim_path=f"/World/env_{self.env_id}/Cell_{count}",
        ...
        for i in range(count_all):
            ...
            wallA_prim = prims.create_prim(
                prim_path=f"/World/env_{self.env_id}/wall_A_{i}",
            ...
            wallB_prim = prims.create_prim(
                prim_path=f"/World/env_{self.env_id}/wall_B_{i}",
            ...
            wallB_up_prim = prims.create_prim(
                prim_path=f"/World/env_{self.env_id}/wall_B_up_{i}",
            ...
            if i < left_num - 1:
                for t in range(beam_num):
                    ...
                    beam_prim = prims.create_prim(
                        prim_path=f"/World/env_{self.env_id}/Beam_{i}_{t}",
            ...
            if left_num < i < left_num + front_num - 1:
                pillar_prim = prims.create_prim(
                    prim_path=f"/World/env_{self.env_id}/Pillar_{i}",
                ...
                for t in range(pillarLen_num):
                    ...
                    pillar_len_prim = prims.create_prim(
                        prim_path=f"/World/env_{self.env_id}/Pillar_len_{i}_{t}",
        ...
        for i in np.arange(x_min + 5, x_max, 8):
            for j in np.arange(y_min + 5, y_max, 6):
                ...
                light_prim = prims.create_prim(
                    prim_path=f"/World/env_{self.env_id}/Light_prim_{count}",
...
class Scence(ObjectStageGenerator):
