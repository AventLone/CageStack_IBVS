...
std::vector<char> RfDetrDetector::readModel(const std::string& file_path)
{
...
bool RfDetrDetector::infer(const cv::Mat& input_image)
{
...
std::vector<float> RfDetrDetector::preprocess(const cv::Mat& image, const cv::Size& input_size,
...
std::vector<Instance> RfDetrDetector::postprocess(const cv::Mat& original_image,
                                                  ...
                                                  const int choose_the_bset_count) const
{
    ...
    for (size_t q = 0; q < mQueriesNum; ++q)
    {
        ...
        const int class_id = static_cast<int>(std::distance(cls_probs.begin(), it));
        ...
        // dets 是归一化坐标
        const int x1 = static_cast<int>((cx - w * 0.5f) * static_cast<float>(img_width));
        const int y1 = static_cast<int>((cy - h * 0.5f) * static_cast<float>(img_height));
        const int x2 = static_cast<int>((cx + w * 0.5f) * static_cast<float>(img_width));
        const int y2 = static_cast<int>((cy + h * 0.5f) * static_cast<float>(img_height));

        cv::Rect bbox = clamp_rect(cv::Rect(x1, y1, x2 - x1, y2 - y1), img_width, img_height);
        if (bbox.width <= 6 || bbox.height <= 6 || bbox.area() < 100)
        {
            continue;
        }
        results.emplace_back(class_id, score, bbox);
    }
